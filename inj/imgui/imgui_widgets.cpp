...
// We provide this internal helper to write your own variant while we figure out how to redesign the base ImageButton() API.
...
// FIXME: This would work nicely if it was a base template, e.g. 'template<T> RadioButton(const char* label, T* v, T v_button)', but I'm not sure how we would expose it..
...
static bool InputTextFilterCharacter(unsigned int *p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback,
    ...
    if (input_source != ImGuiInputSource_Clipboard) {
        ...
        // Filter internals Unicode range. GLFW on OSX seems to send internals characters for special keys like arrow keys (FIXME)
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char *label, float (*values_getter)(void *data, int idx), void *data,
    ...
    // This is currently not exposed in the base API because we need a larger redesign of the whole thing, but in the short-term we are making it available in PlotEx().
    return idx_hovered;
}
